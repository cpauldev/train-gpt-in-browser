...
export function MenuRadioItem({
  ...
  return (
    <MenuPrimitive.RadioItem
      className={cn(
        "grid min-h-8 in-data-[side=none]:min-w-[calc(var(--anchor-width)+1.25rem)] cursor-default grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 rounded-sm py-1 ps-2 pe-2 text-base text-foreground outline-none data-disabled:pointer-events-none data-highlighted:bg-accent data-highlighted:text-accent-foreground data-disabled:opacity-64 sm:min-h-7 sm:text-sm [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className,
      )}
    ...
    >
      {leadingIcon ? (
        <span className="-ms-0.5 flex items-center justify-center">{leadingIcon}</span>
      ) : null}
      <span className="min-w-0">{children}</span>
      <MenuPrimitive.RadioItemIndicator className="justify-self-end -me-0.5">
        <svg
          aria-hidden="true"
          fill="none"
          height="24"
